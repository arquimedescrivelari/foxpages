...
DEFINE CLASS Web AS CUSTOM
	...
	*--- Mime formats
	...
	PROCEDURE Init()
		*--- Debug log
		This.Parent.Log.Add(2,"Web.Init")
	ENDPROC
	...
	PROCEDURE Destroy()
		...
		*--- Debug log
		This.Parent.Log.Add(2,"Web.Destroy")
	ENDPROC

	PROCEDURE Error(nError,cMethod,nLine)
		This.Parent.Log.Add(0,"Web.Error","Method: "+proper(m.cMethod)+CRLF+"Message: "+message())
	ENDPROC
	...
	LOCAL llBadRequest,lcRequest,lnLength,ldFile
		...
		*--- Debug log
		This.Parent.Log.Add(2,"Web.Process")
		...
		*--- Cross-Origin Resource Sharing Access Control
		if !empty(This.Request.Origin) AND This.Request.Request_Scheme+"://"+This.Request.Host # This.Request.Origin
			*--- Open CORS table
			if !used("cors")
				use data\cors in 0
			endif

			*--- Locate the CORS record
			select cors
			locate for site = This.SiteID AND (origin = This.Request.Origin OR origin = "*") AND (uri = This.Request.Document_URI OR uri = "*")

			*--- Access denied
			if eof() OR (This.Request.Method = "GET" AND !cors.get) OR ;
						(This.Request.Method = "POST" AND !cors.post) OR ;
						(This.Request.Method = "PUT" AND !cors.put) OR ;
						(This.Request.Method = "DELETE" AND !cors.delete) OR ;
						(This.Request.Method = "HEAD" AND !cors.head) OR ;
						(This.Request.Method = "OPTIONS" AND !cors.options)

				This.SendError("403","Forbidden","Forbidden")
				return .T.
			else
				*--- Allowed methods
				This.Response.Access_Control_Allow_Methods = iif(cors.get,"GET,","")+;
															 iif(cors.post,"POST,","")+;
															 iif(cors.put,"PUT,","")+;
															 iif(cors.delete,"DELETE,","")+;
															 iif(cors.head,"HEAD,","")+;
															 iif(cors.options,"OPTIONS,","")

				if !empty(This.Response.Access_Control_Allow_Methods)
					This.Response.Access_Control_Allow_Methods = left(This.Response.Access_Control_Allow_Methods,len(This.Response.Access_Control_Allow_Methods)-1)
				endif
				
				*--- Allowed headers
				if !empty(cors.headers)
					This.Response.Access_Control_Allow_Headers = alltrim(cors.headers)
				endif

				*--- Allowed origin
				This.Response.Access_Control_Allow_Origin = alltrim(cors.origin)
			endif
		endif
		...
		*--- Check authorization
		...
		do case
		case ("application/json" $ This.Request.Accept OR "application/xml" $ This.Request.Accept) AND !("text/html" $ This.Request.Accept) && REST Request
			*--- Requested method
			if !inlist(This.Request.Method,"GET","POST","PUT","DELETE","HEAD","OPTIONS")
			...
			*--- Verify if URI exists
			if !file(This.Directory+This.Request.Document_URI+".fxp") AND !file(This.Directory+This.Request.Document_URI+".prg")
				*--- If URI don't exists perhaps it's the ID
				if !file(This.Directory+substr(This.Request.Document_URI,1,rat("/",This.Request.Document_URI)-1)+".fxp") AND ;
				   !file(This.Directory+substr(This.Request.Document_URI,1,rat("/",This.Request.Document_URI)-1)+".prg")
			...
			if This.Request.Method = "OPTIONS"
				*--- Cross-Origin Resource Sharing Preflight
				if type("This.Request.Access_Control_Request_Method") = "C"
					*--- Open CORS table
					if !used("cors")
						use data\cors in 0
					endif
					...
					*--- Locate the CORS record
					...
					if eof()
						...
						*--- CORS found
						...
						This.Response.Status_Description = "No Content"
	...
	LOCAL llAuthorized,lcData,lcUserName,lcPassword,lcRealm,lcNonce,lcURI,lcResponse,lcA1,lcA2,lcResult
		...
		*--- Directory requires authorization
		if !empty(realms.type)
			*--- Debug log
			This.Parent.Log.Add(2,"Web.Authorize")
	...
	PROCEDURE Redirect(URI AS String,Permanent AS Boolean)
		*--- Debug log
		This.Parent.Log.Add(2,"Web.Redirect")
	...
	LOCAL Cookie
		...
		*--- Access-Control-Allow-Headers
		if !empty(This.Response.Access_Control_Allow_Headers)
			This.Response.Header = This.Response.Header + "Access-Control-Allow-Headers: "+alltrim(This.Response.Access_Control_Allow_Headers)+CRLF
		endif

		*--- Access-Control-Allow-Methods
		if !empty(This.Response.Access_Control_Allow_Methods)
			This.Response.Header = This.Response.Header + "Access-Control-Allow-Methods: "+alltrim(This.Response.Access_Control_Allow_Methods)+CRLF
		endif

		*--- Access-Control-Allow-Origin
		if !empty(This.Response.Access_Control_Allow_Origin)
			This.Response.Header = This.Response.Header + "Access-Control-Allow-Origin: "+alltrim(This.Response.Access_Control_Allow_Origin)+CRLF
		endif
		...
		*--- Content-Length
		if This.Response.Status_Code # "204" AND This.Response.Transfer_Encoding # "chunked"
	...
	LOCAL lnLine,lcLine,lcName,lcValue,lcRemaining
		...
		*--- Debug log
		This.Parent.Log.Add(2,"Web.GetCookies")
	...
	LOCAL lnLine,lcLine,lcName,lcValue
		...
		*--- Debug log
		This.Parent.Log.Add(2,"Web.GetVariables")
...
DEFINE CLASS Request AS CUSTOM
	...
	*--- Resquest objects
	...
	PROCEDURE Error(nError,cMethod,nLine)
		*--- Debug log
		This.Parent.Parent.Log.Add(0,"Web.Request.Error","Method: "+proper(m.cMethod)+CRLF+"Message: "+message())
	ENDPROC
ENDDEFINE
